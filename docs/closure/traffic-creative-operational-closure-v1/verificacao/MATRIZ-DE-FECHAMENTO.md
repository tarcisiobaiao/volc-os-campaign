...
| Contrato próprio de PMax | **PROVADO** | `volc_ads/campanha/pmax.py` — **1.172 linhas**, commitado em `73e15fa` ("o quarto canal ganha contrato próprio — e ele não é o de Search"). Provas em `volc_ads/campanha/testes_pmax.py` (`8270d48`): **medido por mim, 50 passed em 2,37s**. Importa `PMAX_FIELD_REQUIREMENTS` e `evaluate_asset_group_coverage` da observabilidade, tem `sondar_proto_v25()` com serialização real, e um `ReciboDeMensuracao` cuja **ausência é significativa** | Worker canais |
