...
**Push:** não houve. **Merge:** não houve. **Main:** intocada. **Supabase oficial:** intocado.
...
A contraprova com duas sessões reais e barreiras por advisory lock reproduziu o rebaixamento: D0 commitado por último gravava `origem='D0'` por cima de D-1. A correção é o mínimo que a prova pediu: UPSERT condicional atômico, locks transacionais na ordem 120404 → 120405, fato antes do recibo na mesma transação.
...
- `pg_advisory_xact_lock(120404, hashtext('v12_04:exec:' || execucao_chave))` e, em seguida, `pg_advisory_xact_lock(120405, hashtext('v12_04:idemp:' || chave_idempotencia))`. Locks de transação, nunca de sessão.
...
- Linhas do lote ordenadas por identidade canônica para ordem de lock determinística.
- Sem table lock, sem retry interno, RPC única, RLS/FORCE, `SECURITY DEFINER`, `search_path` fixo e ledger append-only preservados.

## 4. Estado honesto de P10-T17

A concorrência atômica está **provada em Postgres descartável**. A migration **continua não aplicada** oficialmente (`supabase/migrations/README.md` nesta base ainda registra “não aplicada”; esta lane não consultou produção). Ativação n8n, canário e heartbeat continuam fora de escopo.
...
| Arquivo | Natureza |
|---|---|
| `supabase/migrations/v12_04_gads_fato_canonico_dia.sql` | UPSERT condicional + locks transacionais |
| `scripts/provar-concorrencia-v12_04.py` | Runner A–N, sessões reais, barreiras, SQLSTATE |
