...
import { ArrowLeft, Plus, Edit, Trash2, Save, BarChart3, DollarSign, Users, Settings, Building, Shield, AlertTriangle, ChevronLeft, ChevronRight, Calendar, Lightbulb, X, Percent, Layers, Wallet } from 'lucide-react';
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              id="amount"
              type="number"
              aria-label="Percentual do imposto no mês"
              value={amount}
          ...
          <div className="flex items-center space-x-2">
            <input
              ...
              className="rounded border border-input accent-primary"
            />
    ...
    <Layout>
      <div className={`${isMobile ? 'p-4' : 'p-6'} space-y-8 max-w-7xl mx-auto`}>
        ...
        <div className="reveal" style={{ ['--i' as any]: 0 }}>
          <div className={`flex ${isMobile ? 'flex-col items-start' : 'items-center'} gap-3 mb-2`}>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(-1)}
          ...
          <div className={`${isMobile ? '' : 'ml-11'}`}>
            <div className={`flex ${isMobile ? 'flex-col items-start' : 'items-center'} gap-3`}>
              <h1 className={`font-display font-bold tracking-tight leading-[1.05] ${isMobile ? 'text-[1.7rem]' : 'text-4xl'}`}>
                Configurações de <span className="text-foreground">Custos</span>
              </h1>
              {isAdmin() && (
            ...
            <p className={`mt-3 text-muted-foreground ${isMobile ? 'text-sm' : ''}`}>
              ...
              {!canEditCosts() && (
                <span className="text-warning ml-2">• Visualização apenas</span>
              )}
            </p>
          </div>
        </div>
        ...
        {/* Month Selector */}
        <Card className="relative overflow-hidden shadow-card hover-lift reveal" style={{ ['--i' as any]: 1 }}>
          <span className="pointer-events-none absolute inset-x-0 top-0 h-0.5 bg-info" />
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2">
              <span className="rounded-md bg-info/10 text-info p-1.5"><Calendar className="h-4 w-4" /></span>
              Período de Referência
            </CardTitle>
            <CardDescription>
              Navegue pelos meses para visualizar custos históricos
              {isCurrentMonth() && (
                <span className="text-info font-medium ml-2">• Mês atual</span>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className={`flex ${isMobile ? 'flex-col w-full' : 'items-center'} gap-4`}>
                ...
                <div className={`flex items-center gap-2 bg-muted/40 rounded-lg border border-border p-1 ${isMobile ? 'w-full justify-between' : ''}`}>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <div className={`flex items-center gap-2 ${isMobile ? 'px-2' : 'px-4'} py-1 ${isMobile ? 'min-w-0 flex-1' : 'min-w-[180px]'} justify-center`}>
                    <span className={`font-display font-medium ${isMobile ? 'text-base' : 'text-lg'}`}>
                      {formatMonthDisplay(selectedMonth)}
                    </span>
                    {isCurrentMonth() && (
                      <Badge variant="default" className="bg-info/12 text-info hover:bg-info/12 border-transparent text-xs px-2 py-0.5">
                        Atual
                      </Badge>
                    )}
                  </div>
          ...
          <div className="flex justify-center">
            <Dialog open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen}>
              <DialogTrigger asChild>
                <Button size="lg" className="bg-primary hover:bg-primary/90 shadow-card hover-lift">
        ...
        {/* Taxes Section */}
        <Card className="relative overflow-hidden shadow-card hover-lift reveal" style={{ ['--i' as any]: 2 }}>
          <span className="pointer-events-none absolute inset-x-0 top-0 h-0.5 bg-warning" />
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <span className="rounded-md bg-warning/10 text-warning p-1.5"><Percent className="h-4 w-4" /></span>
              Impostos - Simples Nacional
            </CardTitle>
            <CardDescription>
              Alterações no percentual são válidas a partir do mês atual
              {!isCurrentMonth() && (
                <span className="text-warning font-medium ml-2">• Visualizando mês anterior</span>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'} gap-6`}>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-medium bg-warning/12 text-warning px-2.5 py-1 rounded-full w-fit">
                ...
                <div className="text-xs text-muted-foreground space-y-1">
                  {taxHistory.map((h, index) => (
                    <div key={`${h.month}-${index}`} className="flex justify-between">
                      <span>{h.month}:</span>
                      <span className="tabular font-medium text-foreground">{h.percentage}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className={`flex ${isMobile ? 'flex-col' : 'justify-between items-center'} pt-2 gap-2`}>
              <Button
                variant="outline"
                size="sm"
                className={`bg-warning/10 border-warning/20 hover:bg-warning/20 text-warning ${isMobile ? 'w-full touch-target' : ''}`}
              ...
              {!isCurrentMonth() && (
                <div className={`flex items-center gap-2 ${isMobile ? 'text-xs' : 'text-sm'} text-warning ${isMobile ? 'justify-center' : ''}`}>
        ...
        <Card className="relative overflow-hidden shadow-card hover-lift reveal" style={{ ['--i' as any]: 3 }}>
        ...
        {categories.length > 0 && (
          <div className="flex items-center gap-3 pt-2">
            <span className="kicker whitespace-nowrap">Categorias de custo</span>
        ...
        {categories.map((category, categoryIndex) => {
          ...
          return (
            <Card
              key={category.id}
              style={{ ['--i' as any]: categoryIndex }}
              className="relative overflow-hidden shadow-card hover-lift reveal border-l-4 border-l-primary"
            >
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex-1">
                  {isEditingThis ? (
                    <div className="flex items-center gap-2 mb-2">
                      <span className="rounded-md bg-primary/10 text-primary p-1.5"><Layers className="h-4 w-4" /></span>
                      <Input
                  ...
                  <CardDescription>
                    Subtotal: <span className="tabular font-medium text-foreground">R$ {categoryTotal.toFixed(2).replace('.', ',')}</span>
              ...
              <CardContent>
                <div className="space-y-3">
                  {categoryCosts.map(cost => (
                    <div key={cost.id} className="flex items-center justify-between p-4 border border-border rounded-lg hover:bg-muted/40 transition-colors">
                      <div className="flex items-center gap-4 flex-1">
                        <div className={`h-2 w-2 rounded-full ${cost.is_active ? 'bg-success' : 'bg-muted-foreground/50'}`}></div>
                        <div className="flex-1">
                          <div className="font-medium">{cost.name}</div>
                          {!cost.is_active && (
                            <span className="inline-block mt-1 text-xs bg-warning/12 text-warning rounded-full px-2 py-0.5">Inativo</span>
                          )}
                        </div>
                        <div className="text-right">
                      ...
                      {canEditCosts() && (
                        <div className="flex gap-1 ml-4">
                          ...
                          <DialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" aria-label="Editar lançamento">
        ...
        {categories.length === 0 && (
          <Card className="shadow-card">
            <CardContent className="text-center py-12">
              ...
              <h3 className="font-display text-lg font-medium mb-2">Nenhuma categoria cadastrada</h3>
        ...
        {/* Total Summary */}
        <Card className="relative overflow-hidden shadow-elevated hover-lift">
          <CardContent className="p-8">
            <div className="text-center space-y-4">
              <div className="flex items-center justify-center gap-2">
                <span className="rounded-md bg-primary/10 text-primary p-1.5"><Wallet className="h-4 w-4" /></span>
                <span className="kicker">
                  Total Custos Operacionais
                </span>
              </div>
              <div className="space-y-2">
                <div className="font-display text-4xl font-bold tabular tracking-tight">
                ...
                <div className="space-y-1">
                  ...
                  {selectedProjectsForCosts.length > 0 && (
                    <p className="text-sm text-success font-medium">
                      ...
                      - <span className="tabular">R$ {costPerProject.toFixed(2).replace('.', ',')}</span> por projeto
                    </p>
                  )}
                </div>
              </div>
              ...
              {categories.length > 0 && (
              <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3'} gap-4 pt-4 border-t border-border`}>
                  {categories.slice(0, 3).map((category, index) => {
                    const accents = ['text-info', 'text-success', 'text-primary'];
                    const categoryCosts = groupedCosts[category.name] || [];
                    return (
                      <div key={category.id} className="text-center space-y-1">
                        <div className={`kicker ${accents[index] || 'text-primary'}`}>
                          {category.name}
                        </div>
        ...
        <div className={`flex flex-wrap gap-4 ${isMobile ? 'flex-col' : 'justify-center'}`}>
          {canEditCosts() && (
            <Button
              ...
              size="lg"
              className={`shadow-card hover-lift ${isMobile ? 'w-full touch-target' : ''}`}
            >
            <Save className="h-5 w-5 mr-2" />
              Recarregar Dados
          </Button>
          )}
          <Button variant="outline" size="lg" className={`shadow-card hover-lift ${isMobile ? 'w-full touch-target' : ''}`}>
