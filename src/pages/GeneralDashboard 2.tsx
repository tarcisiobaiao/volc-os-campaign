...
import { useSupabaseData } from "@/services/supabaseDataService";
import { useAuth } from "@/contexts/AuthContext";
import { format } from "date-fns";
import { formatCurrencySync, preloadExchangeRate } from "@/utils/currencyUtils";
...
export default function GeneralDashboard() {
  ...
  const { projects, dailyMetrics, summary, loading, error, refresh } = useSupabaseData();
  const [exchangeRate, setExchangeRate] = useState<number>(5.50);
  const [userProfile, setUserProfile] = useState<{full_name?: string, first_name?: string, name?: string} | null>(null);
  const { user } = useAuth();
  ...
  const getUserFirstName = () => {
    ...
    // Temporary hardcode for Tarcisio while debugging
    if (user.email === 'tarcisio.bely@example.com' || user.email?.includes('tarcisio')) {
      return "Tarcisio";
    }
  ...
  useEffect(() => {
    // Load exchange rate and simulate loading
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        if (error) {
          console.log('Profile not found in users table:', error);
          
          // Try alternative ID field if needed
          ...
          console.log('Alternative query by email result:', { profileAlt, errorAlt });
  ...
  // Função para formatar valores de REVENUE (USD → BRL)
  const formatRevenue = (usdValue: number) => {
    return formatCurrencySync(usdValue, exchangeRate);
  };
                ...
                <BarChart data={dailyMetrics}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: number, name: string) => {
                      ...
                      if (name === "revenue") {
                        return [formatRevenue(value), "Receita"];
                      }
