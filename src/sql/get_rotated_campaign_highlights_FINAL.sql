-- VERSÃO FINAL COM CRITÉRIOS RELAXADOS
-- Gasto mínimo: $5 (ao invés de $20)
-- Dias mínimos: 2 (ao invés de 3)
-- Variação ROAS: 10% (ao invés de 15%)
...
BEGIN
  ...
  metrics AS (
    ...
    HAVING COUNT(*) >= 2 AND AVG(spend) > 5  -- RELAXADO
  ),
  ...
  -- ALERTAS TÉCNICOS (GAM Match Rate)
  tech_alert_list AS (
    SELECT
      m.campaign_id,
    ...
    JOIN public.campaigns c ON c.id = m.campaign_id::text
    JOIN public.projects p ON p.id::text = c.project_id::text
    WHERE m.rn <= 2
  ...
  top_high AS (
    ...
    FROM calculated
    WHERE var_roas_pct >= 0.10  -- RELAXADO: 10%
  ...
  top_low AS (
    ...
    FROM calculated
    WHERE var_roas_pct <= -0.10  -- RELAXADO: -10%
      OR (profit_daily_trend < -5 AND var_roas_pct < 0)
...
COMMENT ON FUNCTION get_rotated_campaign_highlights IS 'Retorna campanhas destacadas com rotação de 5 dias (critérios relaxados: 10% variação, $5 gasto mínimo)';
