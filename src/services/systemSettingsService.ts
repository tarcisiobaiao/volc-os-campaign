import { secureApi, type ChaveConfiguracao } from '@/lib/secureApi';

/**
 * As chaves que este serviço conhece.
 *
 * A lista existe DUAS vezes de propósito — aqui e em
 * `api/_lib/configuracoes.js`. A daqui é conveniência de tipo; a de lá é o
 * controle. Uma lista no cliente não protege nada: quem chama a API não passa
 * pelo TypeScript.
 */
...
class SystemSettingsService {
  ...
  async getSetting(key: ChaveConfiguracao): Promise<string | null> {
    try {
      // Check cache first
      ...
      const data = await secureApi.getSettings([key]);
   ...
   * Grava uma chave.
   *
   * `dollar_exchange_rate` NÃO passa por aqui: o servidor recusa. Ela muda por
   * `setExchangeRate()`, porque a cotação e o recálculo do mês precisam
   * acontecer na mesma transação — gravar só a taxa deixaria as receitas do
   * mês convertidas pela taxa anterior.
   */
  async setSetting(key: ChaveConfiguracao, value: string): Promise<boolean> {
    try {
      await secureApi.setSettings([{ key, value }]);
  ...
  /**
   * Grava a cotação. O carimbo de `last_currency_update` é feito pelo servidor
   * na mesma requisição — não é responsabilidade do cliente encadear duas
   * escritas e torcer para as duas darem certo.
   */
  async setExchangeRate(rate: number): Promise<boolean> {
    try {
      await secureApi.setExchangeRate(rate);
      this.cache.delete('setting_dollar_exchange_rate');
      this.cacheExpiry.delete('setting_dollar_exchange_rate');
      this.cache.delete('setting_last_currency_update');
      this.cacheExpiry.delete('setting_last_currency_update');
      return true;
    } catch (error) {
  ...
  async updateCurrencyConfig(config: Partial<CurrencyConfig>): Promise<boolean> {
    try {
      ...
      if (config.exchangeRate !== undefined) {
        promises.push(this.setExchangeRate(config.exchangeRate));
      }
  ...
  // `getSettingsByCategory` foi removido em 24/08/2026.
  //
  // Ele fazia `select('*')` em `system_settings` filtrando por categoria, pelo
  // proxy que aceitava qualquer tabela. A varredura do frontend não achou
  // nenhum consumidor, e servir "todas as chaves de uma categoria" exigiria
  // devolver chaves que ninguém listou — o oposto da lista fechada que
  // `api/_lib/configuracoes.js` passou a aplicar. Se a necessidade voltar,
  // ela nasce como rota nomeada com as chaves declaradas.

  // Batch update multiple settings
  async updateMultipleSettings(updates: Array<{ key: ChaveConfiguracao; value: string }>): Promise<boolean> {
    try {
      // Uma requisição só: o servidor grava a lista inteira e devolve o que
      // gravou. N requisições paralelas dariam N chances de sucesso parcial
      // silencioso.
      await secureApi.setSettings(updates);
...
export const useSystemSettings = (keys: ChaveConfiguracao[]) => {
  ...
  const updateSetting = async (key: ChaveConfiguracao, value: string) => {
